...
class SimulatedAnnealing:
    ...
    def matyas_function(self, x, y):
        return 0.26 * (x ** 2 + y ** 2) - 0.46 * x * y

    def eggholder_function(self, x, y):
        term1 = -(y + 47) * math.sin(math.sqrt(abs(y + x / 2 + 47)))
        term2 = -y * math.sin(math.sqrt(abs(x - (y + 47))))

        return term1 + term2

    def dixon_price_function(self, x, y):
    ...
    def choose_function(self, function_name, x, y):
        if (function_name == "himmelblau"):
            ...
            return self.matyas_function(x, y)
        return self.himmelblau_function(x, y)
...
if __name__ == '__main__':
    ...
    three_hump_camel_params = {
        'temp': 1000,
        'cooling_rate': 0.95,
        'iterations': 150,
        'local_searches': 10,
        'multiplier': [0.5, 0.1],
        'function_name': 'three_hump_camel_function',
        'lower_bound_x': -1,
        'upper_bound_x': 1,
        'lower_bound_y': -1,
        'upper_bound_y': 1
    }
    ini = three_hump_camel_params
    for i in range(10):
